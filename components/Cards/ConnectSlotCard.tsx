...
const ConnectSlotCard = observer(({ data }) => {
  const { slots, setConnectSlots } = useSlotsStore();
  const [selectedDate, setSelectedDate] = useState(slots?.connect?.date || data[0]?.date || '');

  useEffect(() => {
    setConnectSlots({ date: selectedDate });
  }, [selectedDate, setConnectSlots]);

  const handleDateSelect = (date) => {
    setSelectedDate(date);
    setConnectSlots({ date });
  };
